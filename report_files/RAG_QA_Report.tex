...
achieving higher Mean Reciprocal Rank (MRR) across configurations. The baseline LLM-only achieves 
the highest fact-checking accuracy, suggesting that modern LLMs possess strong general scientific 
knowledge. However, RAG methods are sensitive to retrieval quality: when retrieval fails to 
...
~\cite{lewis2020retrieval} combine parametric generation with
...
~\cite{karpukhin2020dense} introduce DPR, a dual-encoder
...
DistilBERT (\cite{sanh2020distilbert}) is a distilled version of BERT
 that retains 97\% of performance while being 40\% smaller
...
Four computational approaches are evaluated for scientific fact-checking and are represented in Figure~\ref{fig:approaches}:
...
\textbf{Generator component} The language model component utilizes Qwen 2.5 7B (quantized) served locally via
...
candidate pool sizes and context depths. The ranking is evaluated based on whether the top-$K$ (with k=3, 5 and 10)

\textbf{Fact-checking accuracy:} the proportion of correct predictions (label outputs from the LLM generator) 
relative to ground-truth labels in the SciFact dataset, indicating the system's ability to correctly verify 
scientific claims.

\section{Results}

This section presents both qualitative and quantitative results. We first analyze example outputs, 
then report retrieval metrics and end-to-end accuracy.
...
\caption{Example 1: Outputs for a claim under the four approaches baseline, oracle, DPR, and DPR+reranker.}  
\label{fig:ex1}
\end{figure}

Outputs displayed in Figure~\ref{fig:ex1} and Figure~\ref{fig:ex2} highlight key behavioral 
patterns observed in the dataset. The baseline may produce incorrect predictions without 
grounding and may hallucinate in reasoning. When context is provided (oracle), 
...
\caption{Example 2: Outputs for a claim under the four approaches baseline, oracle, DPR, and DPR+reranker.}
\label{fig:ex2}
\end{figure}
...
Figure~\ref{fig:acc} shows that the oracle configuration (LLM with gold evidence) achieves 
...
Performance varies with context depth: at lower K values (k=3, 5), baseline methods slightly 
outperform RAG approaches, but at K=10, RAG performance increases and approaches baseline results. 
This indicates that RAG benefits from larger candidate pools. While RAG improves factual grounding 
through evidence citations, it remains sensitive to retrieval quality, incorrect or irrelevant 
...
QA accuracy \cite{karpukhin2020dense} and validates the core RAG principle of grounding 
